  ...
  void fetchDummyData() {
    List<Map<String, String>> dummyData = List.generate(
      36,
          (index) => {
        "Offer Id": "${index + 1}",
        "Offer Name": "Shop ${index + 1}",
        "Offer Image": "Image ${index + 1}",
        "Type": "Type ${index % 3 + 1}",
        "Offer Description": "Address ${index + 1}",
        "Original Price": "${(index + 1) * 100}",
        "Discount Price": "${(index + 1) * 80}",
        "Brand Name": "Brand ${(index % 5) + 1}",
        "Created At": "2025-03-0${(index % 9) + 1}",
        "Shop Name": "Shop Name ${index + 1}",
        "Shop Address": "Shop Address ${index + 1}",
        "Owner Contact": "98765432${index % 10}",
        "Rejected Count": "${(index % 5) + 1}",
        "Rejection Reason": "Reason ${index % 3 + 1}",
      },
    );
...
class RejectedOffersScreen extends StatelessWidget {
  RejectedOffersScreen({super.key});

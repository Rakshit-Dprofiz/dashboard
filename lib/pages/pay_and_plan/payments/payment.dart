...
/// Controller for managing payment data and state
  ...
  void fetchDummyData() {
    List<Map<String, String>> dummyData = List.generate(
      36,
          (index) => {
        "Id": "${index + 1}",
        "Plan Type": "Type ${index % 3 + 1}",
        "Invoice Id": "INV${index + 1000}",
        "Payment Reference": "REF${index + 500}",
        "Payment Time": "2025-03-0${(index % 9) + 1} 10:00 AM",
        "Plan Expiry Date": "2025-06-0${(index % 9) + 1}",
        "Status": index % 2 == 0 ? "Active" : "Expired",
        "Plan Name": "Premium ${index % 4 + 1}",
        "Shop Name": "Shop ${index + 1}",
        "Shop Address": "Address ${index + 1}",
        "Shop Pin Code": "40000${index % 10}",
        "Shopkeeper Name": "Owner ${index + 1}",
        "Shopkeeper Email": "owner${index + 1}@shop.com",
        "Shopkeeper Phone Number": "98765432${index % 10}",
      },
    );
...
/// Main UI for displaying the payment table.
